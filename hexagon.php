<?php
/**
 * hexagon.php - Hexagon icon generation endpoint
 * Generates hexagonal icons with customizable colors and border
 */

require("functions.php");
require_once('./error_handler.php');
require_once('./types/TypeHexagon.php');

$startTime = microtime(true);

write_string_to_file(LOG_FILE, "START: ---------- hexagon icon ----------");

// Convert GET parameters to lowercase for case-insensitive processing
$_GET_lower = array_change_key_case($_GET, CASE_LOWER);

// Validate parameters
$validationErrors = validateHexagonParams($_GET_lower);
...
$iconFilename = getHexagonFilename($_GET_lower);
...
if (file_exists($iconFilenameFullPath)) {
    ...
    // Generate the icon
    $image = generateHexagonIcon($_GET_lower);
...
serveIconFromCache($iconFilenameFullPath, $iconFilename, $cacheHit, 'hexagon');

write_string_to_file(LOG_FILE, "END: ---------- hexagon icon ----------");
