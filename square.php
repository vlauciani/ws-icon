<?php
/**
 * square.php - Square icon generation endpoint
 * Generates square icons with customizable colors and border
 */

require("functions.php");
require_once('./error_handler.php');
require_once('./types/TypeSquare.php');

$startTime = microtime(true);

write_string_to_file(LOG_FILE, "START: ---------- square icon ----------");

// Convert GET parameters to lowercase for case-insensitive processing
$_GET_lower = array_change_key_case($_GET, CASE_LOWER);

// Validate parameters
$validationErrors = validateSquareParams($_GET_lower);
...
$iconFilename = getSquareFilename($_GET_lower);
...
if (file_exists($iconFilenameFullPath)) {
    ...
    // Generate the icon
    $image = generateSquareIcon($_GET_lower);
...
serveIconFromCache($iconFilenameFullPath, $iconFilename, $cacheHit, 'square');

write_string_to_file(LOG_FILE, "END: ---------- square icon ----------");
