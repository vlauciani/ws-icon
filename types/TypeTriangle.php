<?php
/**
 * TypeTriangle.php - Triangle icon generator
 */

// Validate and get parameters
function validateTriangleParams($params) {
    $errors = [];
...
function getTriangleFilename($params) {
    ...
    return "triangle__bgcolor-{$bgColor}__label-{$label}__labelcolor-{$labelColor}__bordercolor-{$borderColor}__bordersize-{$borderSize}__fontsize-{$fontSize}.png";
}

// Generate triangle icon
function generateTriangleIcon($params) {
    $bgColor = isset($params['bgcolor']) && $params['bgcolor'] !== 'undefined' ? $params['bgcolor'] : 'FFFFFF';
    $labelColor = isset($params['labelcolor']) && $params['labelcolor'] !== 'undefined' ? $params['labelcolor'] : '000000';
    $borderColor = isset($params['bordercolor']) && $params['bordercolor'] !== 'undefined' ? $params['bordercolor'] : '000000';
    $size = 50; // Default triangle size
    $borderSize = isset($params['bordersize']) ? intval($params['bordersize']) : 3;
    $fontSize = isset($params['fontsize']) ? intval($params['fontsize']) : DEFAULT_FONTSIZE;
    ...
    // Create triangle with background color and border
    $image = createTriangle(
        $size,
        [
            'r' => $bgColorRGB[0],
            'g' => $bgColorRGB[1],
    ...
    // Add label text (adjusted 12 pixels lower for triangle centering, only if label is provided and not empty)
    if (isset($params['label']) && !empty($params['label'])) {
        $label = $params['label'];
        addTextWithOffset($image, $label, $fontSize, [
            'r' => $labelColorRGB[0],
            'g' => $labelColorRGB[1],
            'b' => $labelColorRGB[2]
        ], $size, $size, 0, 12);
