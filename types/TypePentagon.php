<?php
/**
 * TypePentagon.php - Pentagon icon generator
 */

// Validate and get parameters
function validatePentagonParams($params) {
    $errors = [];
...
function getPentagonFilename($params) {
    ...
    return "pentagon__bgcolor-{$bgColor}__label-{$label}__labelcolor-{$labelColor}__bordercolor-{$borderColor}__bordersize-{$borderSize}__fontsize-{$fontSize}.png";
}

// Generate pentagon icon
function generatePentagonIcon($params) {
    $bgColor = isset($params['bgcolor']) && $params['bgcolor'] !== 'undefined' ? $params['bgcolor'] : 'FFFFFF';
    $labelColor = isset($params['labelcolor']) && $params['labelcolor'] !== 'undefined' ? $params['labelcolor'] : '000000';
    $borderColor = isset($params['bordercolor']) && $params['bordercolor'] !== 'undefined' ? $params['bordercolor'] : '000000';
    $size = 50; // Default pentagon size
    $borderSize = isset($params['bordersize']) ? intval($params['bordersize']) : 3;
    $fontSize = isset($params['fontsize']) ? intval($params['fontsize']) : DEFAULT_FONTSIZE;
    ...
    // Create pentagon with background color and border
    $image = createPentagon(
        $size,
        [
            'r' => $bgColorRGB[0],
            'g' => $bgColorRGB[1],
